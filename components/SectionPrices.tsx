...
interface SectionPricesProps {
  onBookingClick: () => void;
}

export default function SectionPrices({ onBookingClick }: SectionPricesProps) {
  return (
    <section className="section" id="prices">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 max-w-3xl mx-auto">
        ...
        <AnimateOnScroll animation="slide-in-left" delay={0}>
          <div className="card relative overflow-hidden border-primary/50">
            <div className="absolute top-0 right-0 px-3 py-1 bg-primary text-background text-xs font-bold rounded-bl-xl">
              Популярное
            </div>
            <div className="pt-4">
              <h3 className="font-display font-bold text-xl mb-1">Пробный урок</h3>
              ...
              <CTAButton onClick={onBookingClick} pulse className="w-full">
                Записаться
              </CTAButton>
            </div>
          </div>
        ...
        <AnimateOnScroll animation="slide-in-right" delay={100}>
          <div className="card">
            <h3 className="font-display font-bold text-xl mb-1">Индивидуальное занятие</h3>
            <p className="text-sm text-muted mb-6">
              Персональная программа под ваши цели
            </p>
            <div className="flex items-baseline gap-1 mb-6">
              <span className="font-display font-extrabold text-4xl text-foreground">1 300</span>
            ...
            <CTAButton onClick={onBookingClick} variant="secondary" className="w-full">
              Записаться
            </CTAButton>
          </div>
        </AnimateOnScroll>
